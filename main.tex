% Compile this file as the main document (e.g. in Overleaf set "Main document" to main.tex).
% This file is identical to literature_review.tex so that compilation produces the Literature Review PDF.
...
\section{Comparative Analysis: How Each Paper Differs and How Ours Differs}

This section clarifies how each reviewed paper differs from the others and how our Smart Bus Accident \& Passenger Health Monitoring System differs from and improves upon them.

\subsection{How the Prior Works Differ}

\textbf{IoT accident detection papers:} \cite{ieee_bus_mgmt} focus on bus management and road accident prevention at the city scale; \cite{ieee_comprehensive_iot_accident} provide a broad survey of IoT accident detection without a bus-specific or passenger-health focus; \cite{ieee_iot_accident_emergency} address vehicle-level accident detection and emergency response but do not consider passenger physiological condition; \cite{ieee_rfid_bus_door} add RFID-based seat and door monitoring for buses but lack accident sensing and passenger health integration. None of these combine bus accident detection with per-passenger health monitoring or AI-based triage.

\textbf{Wearable and health papers:} \cite{ieee_smart_wearable_medical} target medical emergencies that may cause accidents (e.g., driver seizure) rather than post-accident passenger assessment; \cite{ieee_wearable_jacket} track patient health in a medical setting, not inside buses during accidents; \cite{ieee_hrv_stress_wearable} and \cite{ieee_esp32_stress} study HRV-based stress detection but do not apply it to bus accident triage. They do not correlate wearable data with accident events or produce passenger-wise severity maps.

\textbf{IMU and crash papers:} \cite{ieee_crash_imu} use IMU sensors for crash detection only, with no passenger health layer; \cite{ieee_esp32_deep} combine ESP32 and deep learning for collision and obstruction detection but focus on vehicle safety, not human impact. Neither links crash data with wearable-derived passenger condition.

\textbf{Edge and real-time papers:} \cite{ieee_edge_mission} address edge intelligence for mission-critical emergency networks at a conceptual level, without bus or passenger-specific design; \cite{ieee_portable_edge_disaster} target portable edge networks for disaster responders, not in-vehicle gateways; \cite{ieee_realtime_iot_safety} present a general public safety alert system with multi-scenario support (fire, gas, traffic, medical) but do not combine bus crash data with passenger vitals for triage.

\textbf{BLE and in-vehicle papers:} \cite{ieee_ble_automobile}, \cite{ieee_ble_intra_vehicle}, and \cite{ieee_intra_vehicular_iot} focus on BLE and intra-vehicle protocols for vehicle health or diagnostics, not passenger health or accident triage.

\textbf{LLM crisis papers:} \cite{ieee_crisis_iot_edge_llm} and \cite{ieee_urban_emergency_llm} integrate IoT, edge, and LLMs for crisis management but at an urban or policy scale; they do not provide a concrete bus accident + passenger health pipeline or seat-level severity output.

\subsection{How Our Paper Differs and Is Better}

Our Smart Bus Accident \& Passenger Health Monitoring System \emph{unifies} accident detection, passenger health monitoring, and AI-based triage in a single bus-focused pipeline. Specifically:

\begin{itemize}
    \item \textbf{Unified pipeline:} Prior work addresses accident detection \emph{or} passenger health \emph{or} edge intelligence \emph{or} LLM crisis support in isolation. We combine all four: bus-mounted ESP32/IMU accident detection, wearable-derived passenger vitals (HR, HRV, SpO$_2$, motion), gateway aggregation, and Gemini-based passenger-wise severity classification (Normal/Panic/Critical).
    \item \textbf{Human-aware triage:} Unlike \cite{ieee_bus_mgmt,ieee_iot_accident_emergency,ieee_crash_imu}, we do not stop at ``accident detected'' or ``vehicle damaged.'' We produce a passenger-wise severity map that enables responders to prioritize critical passengers before arrival.
    \item \textbf{Bus-specific and seat-level:} Unlike generic vehicle or city-scale systems, we target public buses with seat-based passenger identification and per-seat severity summaries.
    \item \textbf{LLM-driven reasoning:} Unlike rule-based or threshold-only approaches \cite{ieee_crash_imu,ieee_esp32_stress}, we use a cloud-hosted LLM (Gemini) to interpret multi-sensor time series in accident context, yielding natural-language triage summaries.
    \item \textbf{Reproducible prototype:} We provide a Flutter app with CSV-based sensor replay and analytics dashboards, enabling validation of the full pipeline without live hardware during the course phase.
\end{itemize}

\section{Summary}

The reviewed literature shows a clear trend toward integrating vehicle-level accident detection, passenger-level physiological monitoring, edge or gateway-level processing, and intelligent decision support. Our system draws on IoT accident detection \cite{ieee_bus_mgmt,ieee_comprehensive_iot_accident,ieee_iot_accident_emergency}, wearable health and stress assessment \cite{ieee_smart_wearable_medical,ieee_wearable_jacket,ieee_hrv_stress_wearable,ieee_esp32_stress}, IMU and ESP32-based crash detection \cite{ieee_crash_imu,ieee_esp32_deep}, edge and real-time emergency systems \cite{ieee_edge_mission,ieee_portable_edge_disaster,ieee_realtime_iot_safety}, BLE and in-vehicle protocols \cite{ieee_ble_automobile,ieee_ble_intra_vehicle,ieee_intra_vehicular_iot}, and LLM-assisted crisis management \cite{ieee_crisis_iot_edge_llm,ieee_urban_emergency_llm}. Our contribution is a unified bus-focused architecture that produces passenger-wise severity maps via Gemini-based reasoning, filling the gap between generic vehicle safety and human-aware public transport emergency intelligence.
